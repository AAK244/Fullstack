...
beforeEach(async () => {
  ...
  token = jwt.sign(userForToken, SECRET, { expiresIn: '1h' });
  ...
  const userId = (await User.findOne({ username: 'testuser1' }))._id;
...
describe('User API tests', () => {
  ...
  test('A valid user can be added', async () => {
    ...
    assert.strictEqual(usersAtEnd.length, initialUsers.length + 2);
